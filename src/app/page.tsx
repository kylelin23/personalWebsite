...
import SplashScreen, {
  SUBTITLES,
} from "../components/splashScreen/SplashScreen";
...
const FOAM = Array.from({ length: 60 }, (_, i) => ({
...
const ISLE_REF_W = 1440,
  ISLE_REF_H = 900;
const ISLE_MIN_SCALE = 0.2,
  ISLE_MAX_SCALE = 1;
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const touchCapable =
      coarse || "ontouchstart" in window || (navigator.maxTouchPoints || 0) > 0;
  ...
  useEffect(() => {
    ...
    const skipEntrance = skipPosterEntrance || skipNextSplashRef.current;
    skipNextSplashRef.current = false;
    setEntering(!skipEntrance);

    const enterTimeout = skipEntrance
      ? undefined
  ...
  useEffect(() => {
    ...
    function onClick(e: MouseEvent) {
      ...
      for (const isle of islesRef.current) {
        const hitR = Math.max(isle.r * 0.85, 30);
        if (Math.hypot(cx - isle.x, cy - isle.y) < hitR) {
          visitIsland(isle.id);
          return;
        }
      }
    ...
    function onMove(e: MouseEvent) {
      ...
      canvas.style.cursor = islesRef.current.some(
        (i) => Math.hypot(mx - i.x, my - i.y) < Math.max(i.r * 0.85, 30),
      )
        ? "pointer"
        : "default";
    }
    ...
    function drawIsle(isle: Isle, scale: number = 1) {
      ...
      ctx.beginPath();
      ctx.ellipse(x, y, r, r * 0.62 - tideOff * 0.1, 0, 0, Math.PI * 2);
      ...
      sg2.addColorStop(0, "rgba(255,255,255,0.1)");
      ...
      tg2.addColorStop(0, "rgba(255,255,255,0.1)");
      tg2.addColorStop(1, "rgba(0,0,0,0.18)");
      ctx.beginPath();
      ctx.ellipse(x, y, r, r * 0.62, 0, 0, Math.PI * 2);
      ctx.fillStyle = tg2;
      ctx.fill();
      ...
      if (theme === "emerald") {
        ctx.beginPath();
        ctx.ellipse(x, y + r * 0.08, r * 0.62, r * 0.38, 0, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(50,160,50,0.35)";
        ctx.fill();
        for (let sp = 0; sp < 7; sp++) {
          const spx = x - r * 0.02 + Math.sin(sp * 0.8) * r * 0.04,
            spy = y + r * 0.18 + sp * r * 0.06;
          ctx.beginPath();
          ctx.ellipse(spx, spy, r * 0.04, r * 0.025, sp * 0.4, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(180,160,120,${0.55 - sp * 0.04})`;
          ctx.fill();
        ...
        ctx.beginPath();
        ctx.rect(x - r * 0.38, y - r * 0.22, r * 0.76, r * 0.4);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x - r * 0.36, y - r * 0.42, r * 0.72, r * 0.42);
        const hbg = ctx.createLinearGradient(x, y - r * 0.42, x, y);
        hbg.addColorStop(0, "#f5e8c0");
        hbg.addColorStop(1, "#e8d4a0");
        ctx.fillStyle = hbg;
        ctx.fill();
        ...
        for (let sd = 0; sd < 7; sd++) {
          ctx.beginPath();
          ctx.moveTo(x - r * 0.36, y - r * 0.37 + sd * r * 0.06);
          ctx.lineTo(x + r * 0.36, y - r * 0.37 + sd * r * 0.06);
        ...
        ctx.closePath();
        const rfg = ctx.createLinearGradient(x, y - r * 0.75, x, y - r * 0.42);
        rfg.addColorStop(0, "#8a2010");
        rfg.addColorStop(1, "#c03820");
        ctx.fillStyle = rfg;
        ctx.fill();
        ...
        for (let row = 0; row < 5; row++) {
          const ry = y - r * 0.42 - row * r * 0.067;
          const rw = r * 0.42 * (1 - row / 5) * 1.02;
          for (let sh = 0; sh < Math.floor(row * 2 + 3); sh++) {
            const sw = (rw * 2) / (row * 2 + 3);
            const sx2 = x - rw + sh * sw;
            ctx.beginPath();
            ctx.rect(sx2 + 1, ry - r * 0.067, sw - 2, r * 0.067);
            ctx.strokeStyle = "rgba(0,0,0,0.1)";
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        ...
        ctx.beginPath();
        ctx.moveTo(x - r * 0.44, y - r * 0.42);
        ctx.lineTo(x + r * 0.44, y - r * 0.42);
        ctx.strokeStyle = "#f0e8d0";
        ctx.lineWidth = 4;
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x - r * 0.36, y - r * 0.32, r * 0.22, r * 0.32);
        ctx.fillStyle = "#eedca8";
        ctx.fill();
        ctx.strokeStyle = "#c8a860";
        ctx.lineWidth = 1;
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x - r * 0.38, y - r * 0.32);
        ctx.lineTo(x - r * 0.25, y - r * 0.48);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x - r * 0.14, y - r * 0.82, r * 0.1, r * 0.42);
        const chg = ctx.createLinearGradient(x - r * 0.14, 0, x - r * 0.04, 0);
        chg.addColorStop(0, "#8a4828");
        chg.addColorStop(1, "#a05838");
        ctx.fillStyle = chg;
        ctx.fill();
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x - r * 0.16, y - r * 0.84, r * 0.14, r * 0.03);
        ctx.fillStyle = "#6a3018";
        ctx.fill();
        for (let cm = 0; cm < 4; cm++) {
          ctx.beginPath();
          ctx.moveTo(x - r * 0.14, y - r * 0.74 + cm * r * 0.08);
          ctx.lineTo(x - r * 0.04, y - r * 0.74 + cm * r * 0.08);
          ctx.strokeStyle = "rgba(0,0,0,0.12)";
          ctx.lineWidth = 1;
          ctx.stroke();
        }
        for (let sm = 0; sm < 4; sm++) {
          ...
          const smx =
            x - r * 0.09 + Math.sin(tick * 0.04 + sm * 1.2) * r * 0.05;
          const smy = y - r * 0.84 - sa * r * 0.28;
          ctx.beginPath();
          ctx.arc(smx, smy, r * (0.025 + sa * 0.04), 0, Math.PI * 2);
          ctx.fillStyle = `rgba(200,195,190,${0.35 * (1 - sa)})`;
          ctx.fill();
        }
        ctx.beginPath();
        ctx.rect(x - r * 0.18, y - r * 0.04, r * 0.36, r * 0.22);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x - r * 0.22, y - r * 0.04);
        ctx.lineTo(x, y - r * 0.16);
        ctx.lineTo(x + r * 0.22, y - r * 0.04);
        ctx.closePath();
        ctx.fillStyle = "#b03020";
        ctx.fill();
        ctx.strokeStyle = "#8a1e10";
        ctx.lineWidth = 1;
        ctx.stroke();
        for (let pc = 0; pc < 3; pc++) {
          const pcx = x - r * 0.16 + pc * r * 0.16;
          ctx.beginPath();
          ctx.rect(pcx - r * 0.025, y - r * 0.04, r * 0.05, r * 0.22);
          const pcg = ctx.createLinearGradient(
            pcx - r * 0.025,
            0,
            pcx + r * 0.025,
            0,
          );
          pcg.addColorStop(0, "#f5f0e0");
          pcg.addColorStop(0.5, "#e8e0c8");
          pcg.addColorStop(1, "#f0ead8");
          ctx.fillStyle = pcg;
          ctx.fill();
          ctx.strokeStyle = "#c8a860";
          ctx.lineWidth = 0.8;
        ...
        for (let pb = 0; pb < 5; pb++) {
          ctx.beginPath();
          ctx.moveTo(x - r * 0.18, y + r * 0.04 + pb * r * 0.03);
          ctx.lineTo(x + r * 0.18, y + r * 0.04 + pb * r * 0.03);
          ctx.strokeStyle = "rgba(0,0,0,0.06)";
          ctx.lineWidth = 0.8;
          ctx.stroke();
        }
        for (let ps = 0; ps < 3; ps++) {
          ...
          ctx.fillStyle = `rgba(200,180,130,${0.8 - ps * 0.15})`;
          ctx.fill();
        }
        ctx.beginPath();
        ctx.rect(x - r * 0.07, y - r * 0.04, r * 0.14, r * 0.22);
        ctx.fillStyle = "#5a1808";
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x, y - r * 0.04, r * 0.07, Math.PI, 0);
        ctx.fillStyle = "#5a1808";
        ctx.fill();
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x - r * 0.06, y - r * 0.03, r * 0.05, r * 0.08);
        ctx.strokeStyle = "rgba(200,160,60,0.4)";
        ctx.lineWidth = 1;
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x + r * 0.01, y - r * 0.03, r * 0.05, r * 0.08);
        ctx.strokeStyle = "rgba(200,160,60,0.4)";
        ctx.lineWidth = 1;
        ctx.stroke();
        ctx.save();
        ...
        ctx.shadowBlur = 4;
        ctx.beginPath();
        ctx.arc(x + r * 0.06, y + r * 0.06, r * 0.018, 0, Math.PI * 2);
        ctx.fillStyle = "#d4a020";
        ctx.fill();
        ctx.restore();
        ctx.beginPath();
        ctx.arc(x, y - r * 0.04, r * 0.04, 0, Math.PI * 2);
        ctx.strokeStyle = "#2a8a20";
        ctx.lineWidth = 3;
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(x, y - r * 0.04, r * 0.04, 0, Math.PI * 2);
        ...
        ctx.setLineDash([]);
        ctx.beginPath();
        ctx.rect(x + r * 0.1, y - r * 0.36, r * 0.22, r * 0.2);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x + r * 0.21, y - r * 0.36);
        ctx.lineTo(x + r * 0.21, y - r * 0.16);
        ctx.strokeStyle = "#c8a860";
        ctx.lineWidth = 1;
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x + r * 0.1, y - r * 0.26);
        ctx.lineTo(x + r * 0.32, y - r * 0.26);
        ctx.strokeStyle = "#c8a860";
        ctx.lineWidth = 1;
        ctx.stroke();
        ...
        ctx.shadowBlur = wglow ? 14 : 6;
        ctx.beginPath();
        ctx.rect(x + r * 0.11, y - r * 0.35, r * 0.2, r * 0.18);
        ctx.fillStyle = `rgba(255,240,150,${wglow ? 0.35 : 0.2})`;
        ctx.fill();
        ctx.restore();
        ctx.beginPath();
        ctx.rect(x + r * 0.08, y - r * 0.16, r * 0.26, r * 0.04);
        ...
        for (let wf = 0; wf < 6; wf++) {
          ...
          ctx.arc(
            x + r * 0.11 + wf * r * 0.04,
            y - r * 0.18 + Math.sin(tick * 0.04 + wf) * r * 0.008,
            r * 0.02,
            0,
            Math.PI * 2,
          );
        ...
        ctx.beginPath();
        ctx.rect(x - r * 0.32, y - r * 0.36, r * 0.16, r * 0.14);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x - r * 0.24, y - r * 0.36);
        ctx.lineTo(x - r * 0.24, y - r * 0.22);
        ctx.moveTo(x - r * 0.32, y - r * 0.29);
        ctx.lineTo(x - r * 0.16, y - r * 0.29);
        ctx.strokeStyle = "#c8a860";
        ctx.lineWidth = 1;
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x - r * 0.32, y - r * 0.36, r * 0.04, r * 0.14);
        ctx.fillStyle = "rgba(220,160,140,0.5)";
        ctx.fill();
        ctx.beginPath();
        ctx.rect(x - r * 0.2, y - r * 0.36, r * 0.04, r * 0.14);
        ctx.fillStyle = "rgba(220,160,140,0.5)";
        ctx.fill();
        ctx.beginPath();
        ctx.rect(x + r * 0.05, y - r * 0.65, r * 0.14, r * 0.1);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x + r * 0.03, y - r * 0.65);
        ctx.lineTo(x + r * 0.12, y - r * 0.73);
        ctx.lineTo(x + r * 0.21, y - r * 0.65);
        ctx.closePath();
        ctx.fillStyle = "#b03020";
        ctx.fill();
        ctx.beginPath();
        ...
        ctx.fillStyle = "rgba(40,150,40,0.5)";
        ctx.fill();
        for (let gf = 0; gf < 8; gf++) {
          const gfa = (gf / 8) * Math.PI * 2,
            gfr = r * 0.08;
          const gfx = x - r * 0.42 + Math.cos(gfa) * gfr,
            gfy = y + r * 0.05 + Math.sin(gfa) * gfr * 0.6;
          ctx.beginPath();
          ctx.arc(gfx, gfy, r * 0.022, 0, Math.PI * 2);
          ctx.fillStyle = [
            "#ff6080",
            "#ff9020",
            "#ffe040",
        ...
        ctx.lineTo(x + r * 0.42 + treeSway * 0.3, y - r * 0.1);
        ctx.strokeStyle = "#5a3010";
        ctx.lineWidth = r * 0.04;
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x + r * 0.42 + treeSway * 0.2, y - r * 0.0);
        ctx.lineTo(x + r * 0.55 + treeSway, y - r * 0.12);
        ctx.strokeStyle = "#5a3010";
        ctx.lineWidth = r * 0.025;
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x + r * 0.42 + treeSway * 0.3, y - r * 0.08);
        ctx.lineTo(x + r * 0.3 + treeSway, y - r * 0.18);
        ctx.strokeStyle = "#5a3010";
        ctx.lineWidth = r * 0.02;
        ctx.stroke();
        for (let lc = 0; lc < 5; lc++) {
          const lca = (lc / 5) * Math.PI * 2,
            lcr = r * (0.18 + Math.sin(lc * 1.4) * 0.04);
          const lcx = x + r * 0.42 + Math.cos(lca) * lcr + treeSway,
            lcy = y - r * 0.12 + Math.sin(lca) * lcr * 0.65;
          ctx.beginPath();
          ctx.arc(
            lcx,
            lcy,
            r * (0.1 + Math.sin(lc * 2.1) * 0.03),
            0,
            Math.PI * 2,
          );
          ctx.fillStyle = `rgba(${30 + lc * 8},${130 + lc * 10},${20 + lc * 5},0.75)`;
          ctx.fill();
        }
        ctx.beginPath();
        ctx.rect(x + r * 0.22, y + r * 0.32, r * 0.07, r * 0.08);
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.rect(x + r * 0.254, y + r * 0.36, r * 0.06, r * 0.02);
        ctx.fillStyle = "rgba(255,255,255,0.3)";
        ctx.fill();
        ctx.beginPath();
        ctx.rect(x + r * 0.24, y + r * 0.38, r * 0.03, r * 0.08);
        ...
        ctx.shadowBlur = 12;
        ctx.beginPath();
        ctx.arc(x, y - r * 0.16, r * 0.025, 0, Math.PI * 2);
        ctx.fillStyle = "#ffe060";
        ctx.fill();
        ctx.restore();
        ctx.beginPath();
        ctx.rect(x - r * 0.06, y + r * 0.17, r * 0.12, r * 0.04);
        ...
        ctx.font = `bold ${r * 0.025}px serif`;
        ctx.textAlign = "center";
        ctx.fillText("HOME", x, y + r * 0.2);
        ctx.restore();
      } else if (theme == "storm") {
        ctx.beginPath();
        ctx.rect(x - r * 0.46, y - r * 0.46, r * 0.92, r * 0.42);
        const libg = ctx.createLinearGradient(x, y - r * 0.46, x, y - r * 0.04);
        libg.addColorStop(0, "#e8dca8");
        libg.addColorStop(1, "#c8b878");
        ctx.fillStyle = libg;
        ctx.fill();
        ...
        for (let br = 0; br < 5; br++) {
          ctx.beginPath();
          ctx.moveTo(x - r * 0.46, y - r * 0.38 + br * r * 0.07);
          ctx.lineTo(x + r * 0.46, y - r * 0.38 + br * r * 0.07);
          ctx.strokeStyle = "rgba(0,0,0,0.07)";
          ctx.lineWidth = 1;
          ctx.stroke();
        }
        for (let lp = 0; lp < 7; lp++) {
          const lpx = x - r * 0.4 + lp * r * 0.135;
          ctx.beginPath();
          ctx.rect(lpx - 4, y - r * 0.46, 8, r * 0.42);
          const pg = ctx.createLinearGradient(lpx - 4, 0, lpx + 4, 0);
          pg.addColorStop(0, "#efe088");
          pg.addColorStop(0.5, "#d8c870");
          pg.addColorStop(1, "#e8d880");
          ctx.fillStyle = pg;
          ctx.fill();
        ...
        ctx.beginPath();
        ctx.moveTo(x - r * 0.5, y - r * 0.46);
        ctx.lineTo(x, y - r * 0.68);
        ctx.lineTo(x + r * 0.5, y - r * 0.46);
        ctx.closePath();
        ...
        ctx.stroke();
        ctx.save();
        ctx.translate(x, y - r * 0.56);
        ctx.scale(1, 0.6);
        ctx.beginPath();
        ctx.arc(-r * 0.08, 0, r * 0.06, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(160,130,60,0.4)";
        ctx.fill();
        ctx.beginPath();
        ctx.arc(r * 0.08, 0, r * 0.06, 0, Math.PI * 2);
        ctx.fill();
        ...
        ctx.shadowBlur = 3;
        ctx.fillText("LIBRARY", x, y - r * 0.22);
        ctx.restore();
        ctx.beginPath();
        ctx.arc(x, y - r * 0.58, r * 0.07, 0, Math.PI * 2);
        ...
        ctx.lineTo(
          x + Math.cos(ch) * r * 0.04,
          y - r * 0.58 + Math.sin(ch) * r * 0.04,
        );
        ctx.strokeStyle = "#333";
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ...
        ctx.moveTo(x, y - r * 0.58);
        ctx.lineTo(
          x + Math.cos(ch * 12) * r * 0.055,
          y - r * 0.58 + Math.sin(ch * 12) * r * 0.055,
        );
        ctx.strokeStyle = "#333";
        ctx.lineWidth = 1;
        ctx.stroke();
        for (let ls = 0; ls < 3; ls++) {
          ...
          ctx.fillStyle = `rgba(210,190,110,${0.85 - ls * 0.1})`;
          ctx.fill();
        }
        ctx.beginPath();
        ctx.ellipse(
        ...
        ctx.fillStyle = "rgba(80,130,40,0.4)";
        ctx.fill();
        for (let rb = 0; rb < 2; rb++) {
          const rba = (rb / 2) * Math.PI + 0.5;
          ctx.beginPath();
          ctx.rect(
            x - r * 0.3 + Math.cos(rba) * r * 0.14 - r * 0.03,
            y + r * 0.2 + Math.sin(rba) * r * 0.1 - r * 0.01,
            r * 0.06,
            r * 0.025,
          );
        ...
        ctx.shadowBlur = 8;
        ctx.beginPath();
        ctx.arc(x - r * 0.3, y + r * 0.08, 3, 0, Math.PI * 2);
        ctx.fillStyle = "#ffe060";
        ctx.fill();
        ctx.restore();
        ctx.save();
        ctx.translate(x + r * 0.3, y + r * 0.1);
        ctx.rotate(-0.65);
        ...
        ctx.beginPath();
        ctx.moveTo(0, -r * 0.25);
        ctx.bezierCurveTo(r * 0.1, -r * 0.3, r * 0.16, -r * 0.2, 0, -r * 0.25);
        ...
        for (let bp = 0; bp < 3; bp++) {
          ...
          ctx.fillStyle = ["#20a0a0", "#c06020", "#4020c0"][bp];
          ctx.fill();
          ctx.strokeStyle = "rgba(0,0,0,0.2)";
          ctx.lineWidth = 0.5;
          ctx.stroke();
        ...
        ctx.save();
        ctx.translate(x + r * 0.05, y + r * 0.15);
        ctx.rotate(-0.1);
        ctx.globalAlpha = 0.9;
        ctx.beginPath();
        ctx.rect(-r * 0.1, -r * 0.065, r * 0.2, r * 0.075);
        ctx.fillStyle = "#2050c0";
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(-r * 0.1, -r * 0.065);
        ctx.quadraticCurveTo(-r * 0.15, -r * 0.08, -r * 0.1, -r * 0.065);
        ctx.lineTo(-r * 0.1, r * 0.01);
        ctx.closePath();
        ...
        for (let ln = 0; ln < 4; ln++) {
          ctx.beginPath();
          ctx.moveTo(-r * 0.09, (ln - 2) * r * 0.015);
          ctx.lineTo(-r * 0.01, (ln - 2) * r * 0.015);
        ...
        for (let ln = 0; ln < 4; ln++) {
          ctx.beginPath();
          ctx.moveTo(r * 0.01, (ln - 2) * r * 0.015);
          ctx.lineTo(r * 0.09, (ln - 2) * r * 0.015);
          ctx.strokeStyle = "rgba(80,60,40,0.35)";
          ctx.lineWidth = 0.7;
          ctx.stroke();
        }
        ctx.restore();
        ctx.beginPath();
        ctx.arc(x - r * 0.42, y + r * 0.12, r * 0.06, 0, Math.PI * 2);
        ctx.strokeStyle = "rgba(180,160,100,0.75)";
        ctx.lineWidth = 3;
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x - r * 0.375, y + r * 0.165);
        ctx.lineTo(x - r * 0.32, y + r * 0.21);
        ctx.strokeStyle = "rgba(140,120,70,0.75)";
        ctx.lineWidth = 4;
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(x - r * 0.42, y + r * 0.12, r * 0.055, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(180,220,255,0.12)";
        ctx.fill();
      } else if (theme === "golden") {
        ctx.beginPath();
        ctx.rect(x - r * 0.48, y - r * 0.44, r * 0.96, r * 0.4);
        const galg = ctx.createLinearGradient(x, y - r * 0.44, x, y - r * 0.04);
        galg.addColorStop(0, "#eee8f8");
        galg.addColorStop(1, "#ccc0e0");
        ctx.fillStyle = galg;
        ctx.fill();
        ctx.strokeStyle = "#7060a0";
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ...
        ctx.beginPath();
        ctx.rect(x - r * 0.5, y - r * 0.46, r * 1.0, r * 0.04);
        ctx.fillStyle = "#8070b0";
        ctx.fill();
        for (let sp = 0; sp < 4; sp++) {
          const spx = x - r * 0.36 + sp * r * 0.24;
          ctx.save();
          ctx.translate(spx, y - r * 0.44);
          ctx.save();
        ...
        ctx.beginPath();
        ctx.rect(x - r * 0.1, y - r * 0.28, r * 0.2, r * 0.24);
        ctx.fillStyle = "rgba(20,10,40,0.8)";
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x, y - r * 0.28, r * 0.1, Math.PI, 0);
        ctx.fillStyle = "rgba(20,10,40,0.8)";
        ctx.fill();
        ...
        ctx.textAlign = "center";
        ctx.fillText("GALLERY", x, y - r * 0.14);
        ctx.restore();
        for (let ls = 0; ls < 3; ls++) {
          ctx.beginPath();
          ctx.rect(
          ...
          ctx.fillStyle = `rgba(190,175,225,${0.85 - ls * 0.1})`;
          ctx.fill();
        }
        ctx.beginPath();
        ctx.ellipse(
        ...
        ctx.fillStyle = "rgba(90,80,120,0.3)";
        ctx.fill();
        [
          [x - r * 0.42, y + r * 0.2],
          [x - r * 0.25, y + r * 0.14],
          ...
          ctx.beginPath();
          ctx.rect(sx - r * 0.02, sy - r * 0.1, r * 0.04, r * 0.14);
          const scg = ctx.createLinearGradient(
            sx,
            sy - r * 0.1,
            sx,
            sy + r * 0.04,
          );
          scg.addColorStop(0, "#e8e8f0");
          scg.addColorStop(1, "#c8c8d8");
          ctx.fillStyle = scg;
          ctx.fill();
          ...
          ctx.stroke();
          ctx.beginPath();
          ctx.arc(sx, sy - r * 0.1, r * 0.04, 0, Math.PI * 2);
        ...
        ctx.shadowBlur = 8;
        ctx.beginPath();
        ctx.arc(x + r * 0.25, y + r * 0.22, r * 0.08, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(80,140,255,0.4)";
        ctx.fill();
        ctx.restore();
        for (let fj = 0; fj < 5; fj++) {
          const fa = (fj / 5) * Math.PI * 2 + tick * 0.02,
            fh = r * 0.05;
          ctx.beginPath();
          ctx.moveTo(x + r * 0.25, y + r * 0.22);
          ctx.lineTo(
            x + r + 0.25 + Math.cos(fa) * fh,
            y + r * 0.22 + Math.sin(fa) * fh * 0.6,
          );
        ...
        ctx.save();
        ctx.translate(x + r * 0.38, y + r * 0.08);
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(-r * 0.08, -r * 0.18, r * 0.16, r * 0.16);
        ctx.fillStyle = "#f8f4e8";
        ctx.fill();
        ctx.strokeStyle = "#8a6020";
        ctx.lineWidth = 1;
        ctx.stroke();
        const paintHue = (tick * 2) % 360;
        ctx.beginPath();
        ctx.arc(-r * 0.02, -r * 0.1, r * 0.04, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(${paintHue},70%,55%,0.6)`;
        ctx.fill();
        ctx.beginPath();
        ctx.arc(r * 0.1, -r * 0.05, r * 0.04, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(60,40,20,0.6)";
        ctx.fill();
        ctx.beginPath();
        ctx.rect(r * 0.06, r * 0.0, r * 0.08, r * 0.1);
        ctx.fillStyle = "rgba(60,40,20,0.6)";
        ctx.fill();
        ctx.restore();
        const pp = tick * 0.007;
        const px3 = x - r * 0.44 + Math.sin(pp) * r * 0.06,
          py3 = y + r * 0.06 + Math.cos(pp) * r * 0.04;
        ctx.save();
        ctx.translate(px3, py3);
        ctx.rotate(pp * 0.3);
        ctx.globalAlpha = 0.82;
        ctx.beginPath();
        ctx.ellipse(0, 0, r * 0.11, r * 0.08, 0, 0, Math.PI * 2);
        ...
        for (let cs = 0; cs < 12; cs++) {
          const ca = (cs / 12) * Math.PI * 2,
            cr2 = r * (0.1 + Math.random() * 0.15);
          ctx.beginPath();
          ctx.arc(
            x + Math.cos(ca) * cr2,
            y + r * 0.1 + Math.sin(ca) * cr2 * 0.5,
            r * 0.015,
            0,
            Math.PI * 2,
          );
          ctx.fillStyle = `hsla(${cs * 30},80%,55%,0.25)`;
          ctx.fill();
        }
      } else if (theme === "crystal") {
        ctx.beginPath();
        ctx.rect(x - r * 0.46, y - r * 0.46, r * 0.92, r * 0.42);
        const scg2 = ctx.createLinearGradient(x, y - r * 0.46, x, y - r * 0.04);
        scg2.addColorStop(0, "#e8d8c0");
        scg2.addColorStop(1, "#c8b898");
        ctx.fillStyle = scg2;
        ctx.fill();
        ...
        for (let br = 0; br < 6; br++) {
          ctx.beginPath();
          ctx.moveTo(x - r * 0.46, y - r * 0.39 + br * r * 0.065);
          ctx.lineTo(x + r * 0.46, y - r * 0.39 + br * r * 0.065);
          ctx.strokeStyle = "rgba(0,0,0,0.06)";
          ctx.lineWidth = 1;
          ctx.stroke();
        }
        for (let wr = 0; wr < 3; wr++)
          for (let wc = 0; wc < 5; wc++) {
            const wx2 = x - r * 0.4 + wc * r * 0.18,
              wy2 = y - r * 0.42 + wr * r * 0.13;
            const lit = Math.sin(tick * 0.02 + wr * 3 + wc * 2.1) > -0.4;
            ctx.beginPath();
            ctx.rect(wx2, wy2, r * 0.12, r * 0.09);
            ctx.fillStyle = lit
              ? "rgba(255,240,180,0.88)"
            ...
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(wx2 + r * 0.06, wy2);
            ctx.lineTo(wx2 + r * 0.06, wy2 + r * 0.09);
            ctx.moveTo(wx2, wy2 + r * 0.045);
            ctx.lineTo(wx2 + r * 0.12, wy2 + r * 0.045);
        ...
        ctx.font = `bold ${r * 0.08}px Georgia,serif`;
        ctx.textAlign = "center";
        ctx.fillText("CAL POLY", x, y - r * 0.22);
        ctx.fillStyle = "rgba(80,50,20,0.6)";
        ctx.font = `${r * 0.055}px Georgia,serif`;
        ctx.fillText("SAN LUIS OBISPO", x, y - r * 0.14);
        ctx.restore();
        ctx.beginPath();
        ctx.rect(x - r * 0.1, y - r * 0.68, r * 0.2, r * 0.23);
        ...
        ctx.fill();
        ctx.beginPath();
        ctx.rect(x - r * 0.05, y - r * 0.62, r * 0.1, r * 0.08);
        ctx.fillStyle = "rgba(180,210,255,0.5)";
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x, y - r * 0.62, r * 0.05, Math.PI, 0);
        ctx.fillStyle = "rgba(180,210,255,0.5)";
        ctx.fill();
        const bsw = Math.sin(tick * 0.08) * 0.2;
        ctx.save();
        ctx.translate(x, y - r * 0.58);
        ctx.rotate(bsw);
        ctx.save();
        ...
        for (let ss = 0; ss < 3; ss++) {
          ...
          ctx.fillStyle = `rgba(180,155,115,${0.85 - ss * 0.1})`;
          ctx.fill();
        }
        ctx.beginPath();
        ctx.rect(x - r * 0.1, y - r * 0.18, r * 0.2, r * 0.14);
        ctx.fillStyle = "rgba(60,40,15,0.75)";
        ctx.fill();
        ctx.beginPath();
        ctx.arc(x, y - r * 0.18, r * 0.1, Math.PI, 0);
        ctx.fillStyle = "rgba(60,40,15,0.75)";
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(x + r * 0.38, y + r * 0.0);
        ctx.lineTo(x + r * 0.38, y - r * 0.4);
        ctx.strokeStyle = "#c0c0c0";
        ctx.lineWidth = 2.5;
        ctx.stroke();
        ctx.save();
        ctx.translate(x + r * 0.38, y - r * 0.4);
        ctx.beginPath();
        ctx.moveTo(0, 0);
        for (let fw = 0; fw <= 10; fw++) {
          ctx.lineTo(
            fw * r * 0.025,
            Math.sin(fw * 0.8 - tick * 0.12) * r * 0.025,
          );
        }
        ctx.lineTo(r * 0.25, r * 0.05);
        for (let fw = 10; fw >= 0; fw--) {
          ctx.lineTo(
            fw * r * 0.025,
            r * 0.04 + Math.sin(fw * 0.8 - tick * 0.12) * r * 0.025,
          );
        }
        ctx.closePath();
        ...
        for (let fw = 0; fw <= 10; fw++)
          ctx.lineTo(
            fw * r * 0.025,
            r * 0.02 + Math.sin(fw * 0.8 - tick * 0.12) * r * 0.025,
          );
        ctx.lineTo(r * 0.25, r * 0.025);
        ...
        ctx.fillStyle = "rgba(30,140,40,0.45)";
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(x - r * 0.43, y + r * 0.26);
        ctx.lineTo(x - r * 0.07, y + r * 0.26);
        ctx.strokeStyle = "rgba(255,255,255,0.4)";
        ctx.lineWidth = 1;
        ctx.stroke();
        ctx.beginPath();
        ctx.rect(x + r * 0.14, y + r * 0.08, r * 0.24, r * 0.18);
        ctx.strokeStyle = "rgba(200,150,50,0.5)";
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(x + r * 0.26, y + r * 0.08, r * 0.06, 0, Math.PI * 2);
        ...
        ctx.shadowBlur = 4;
        ctx.beginPath();
        ctx.arc(x + r * 0.38, y + r * 0.1, r * 0.025, 0, Math.PI * 2);
        ctx.strokeStyle = "#ff6010";
        ctx.lineWidth = 2;
        ctx.stroke();
        ctx.restore();
        ctx.beginPath();
        ctx.moveTo(x + r * 0.38, y + r * 0.06);
        ctx.lineTo(x + r * 0.38, y + r * 0.02);
        ctx.strokeStyle = "#888";
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ctx.save();
        ctx.translate(x + r * 0.32, y - r * 0.28);
        ctx.rotate(0);
        ctx.globalAlpha = 0.9;
        ctx.beginPath();
        ctx.rect(-r * 0.13, -r * 0.065, r * 0.26, r * 0.09);
        ctx.fillStyle = "#f8f4e8";
        ctx.fill();
        ctx.strokeStyle = "#c8a050";
        ctx.lineWidth = 1.5;
        ctx.stroke();
        ctx.beginPath();
        ctx.ellipse(-r * 0.13, 0, r * 0.03, r * 0.065, 0, 0, Math.PI * 2);
        ...
        ctx.fill();
        ctx.save();
        ctx.shadowColor = "#c03020";
        ctx.shadowBlur = 3;
        ctx.beginPath();
        ctx.arc(r * 0.04, r * 0.01, r * 0.025, 0, Math.PI * 2);
        ctx.fillStyle = "#c03020";
        ctx.fill();
        ctx.restore();
        ctx.beginPath();
        ctx.moveTo(-r * 0.04, -r * 0.065);
        ctx.lineTo(r * 0.04, -r * 0.065);
        ctx.strokeStyle = "#c8a050";
        ctx.lineWidth = 2;
        ctx.stroke();
        ctx.restore();
        ...
        ctx.globalAlpha = 0.85;
        ctx.beginPath();
        ctx.rect(-r * 0.1, -r * 0.012, r * 0.2, r * 0.012);
        ctx.fillStyle = "#1a1a4a";
        ctx.fill();
        ctx.beginPath();
        ctx.rect(-r * 0.075, -r * 0.052, r * 0.15, r * 0.042);
        ctx.fillStyle = "#1a1a4a";
        ctx.fill();
        ctx.beginPath();
        ctx.moveTo(r * 0.075, -r * 0.012);
        ctx.lineTo(r * 0.075 + r * 0.04, r * 0.06);
        ctx.strokeStyle = "#c8a020";
        ctx.lineWidth = 1.8;
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(r * 0.075 + r * 0.04, r * 0.06, 3, 0, Math.PI * 2);
        ctx.fillStyle = "#c8a020";
        ctx.fill();
        ctx.restore();
        ctx.save();
        ctx.translate(x + r * 0.1, y + r * 0.2);
        ctx.rotate(-0.4);
        ctx.globalAlpha = 0.85;
        ctx.beginPath();
        ctx.rect(-2, -r * 0.2, 4, r * 0.2);
        ...
        (
        ...
        ).forEach(([tdx, tdy, tdir, tsc], ti) => {
          const tx = x + r * tdx,
            ty = y + r * tdy,
            sway = qSway * tdir * (0.6 + ti * 0.4);
          ctx.beginPath();
          ctx.ellipse(
            tx,
            ty + r * 0.07 * tsc,
          ...
          ctx.fillStyle = "rgba(30,110,40,0.5)";
          ctx.fill();
          ctx.beginPath();
          ctx.moveTo(tx, ty + r * 0.06 * tsc);
          ...
          ).forEach(([cdx, cdy, crad], ci) => {
            const cx3 = tx + sway + r * cdx * tsc,
              cy3 = ty + r * cdy * tsc;
            ctx.beginPath();
            ctx.arc(cx3, cy3, r * crad * tsc, 0, Math.PI * 2);
            ctx.fillStyle = `rgba(${34 + ci * 9},${112 + ci * 13},${42 + ci * 7},0.88)`;
            ctx.fill();
        ...
        ctx.save();
        ctx.strokeStyle = "rgba(210,190,150,0.32)";
        ctx.lineWidth = r * 0.045;
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.moveTo(x - r * 0.34, y + r * 0.06);
        ctx.quadraticCurveTo(x, y + r * 0.18, x + r * 0.32, y + r * 0.05);
        ctx.stroke();
        ctx.restore();
      } else if (theme === "sunset") {
        ctx.beginPath();
        ctx.moveTo(x - r * 0.045, y - r * 0.04);
        ctx.lineTo(x - r * 0.025, y - r * 0.82);
        ctx.lineTo(x + r * 0.025, y - r * 0.82);
        ctx.lineTo(x + r * 0.045, y - r * 0.04);
        ctx.closePath();
        const twg = ctx.createLinearGradient(x, y - r * 0.82, x, y - r * 0.04);
        twg.addColorStop(0, "#b0b0b8");
        twg.addColorStop(1, "#808090");
        ctx.fillStyle = twg;
        ctx.fill();
        ...
        for (let ts = 0; ts < 7; ts++) {
          const ty2 = y - r * 0.08 - ts * r * 0.105,
            tw = r * (0.12 - ts * 0.014);
          ctx.beginPath();
          ctx.moveTo(x - tw, ty2);
          ctx.lineTo(x + tw, ty2);
          ctx.strokeStyle = "rgba(100,100,110,0.55)";
          ctx.lineWidth = 1.5;
          ctx.stroke();
          ctx.beginPath();
          ctx.moveTo(x - tw, ty2);
          ctx.lineTo(x - (tw - r * 0.012), ty2 - (r * 0.105) / 2);
          ctx.moveTo(x + tw, ty2);
          ctx.lineTo(x + (tw - r * 0.012), ty2 - (r * 0.105) / 2);
        ...
        for (let an = 0; an < 5; an++) {
          const ax2 = (an - 2) * r * 0.06;
          ctx.beginPath();
          ctx.moveTo(x + ax2, y - r * 0.82);
          ctx.lineTo(x + ax2, y - r * (0.9 + Math.abs(an - 2) * 0.02));
          ctx.strokeStyle = "#909098";
          ctx.lineWidth = 2;
          ctx.stroke();
        }
        if (Math.sin(tick * 0.18) > 0.5) {
          ctx.save();
          ctx.shadowColor = "#ff2020";
          ctx.shadowBlur = 14;
          ctx.beginPath();
          ctx.arc(x, y - r * 0.92, 4, 0, Math.PI * 2);
          ctx.fillStyle = "#ff2020";
          ctx.fill();
          ctx.restore();
        }
        for (let sw = 0; sw < 4; sw++) {
          const swp = (tick * 0.018 + sw * 0.25) % 1;
          const swr = r * (0.08 + swp * 0.55);
          ctx.beginPath();
          ctx.arc(x, y - r * 0.55, swr, Math.PI * 1.15, Math.PI * 1.85);
          ctx.strokeStyle = `rgba(80,200,255,${0.45 * (1 - swp)})`;
          ctx.lineWidth = 1.5;
          ctx.stroke();
          ctx.beginPath();
          ctx.arc(
        ...
        ctx.beginPath();
        ctx.rect(x - r * 0.42, y - r * 0.32, r * 0.34, r * 0.28);
        ...
        crg.addColorStop(0, "#5a5a70");
        crg.addColorStop(1, "#3a3a50");
        ctx.fillStyle = crg;
        ctx.fill();
        ...
        ctx.textAlign = "center";
        ctx.fillText("CTRL", x - r * 0.25, y - r * 0.12);
        ctx.restore();
        ctx.save();
        ctx.translate(x - r * 0.28, y - r * 0.32);
        ctx.rotate(0.5);
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.arc(0, 0, r * 0.1, Math.PI * 0.1, Math.PI * 0.9);
        ctx.closePath();
        ...
        for (let ph = 0; ph < 3; ph++) {
          const phx = x + r * (0.08 + ph * 0.12),
            phy = y - r * 0.06;
          ctx.beginPath();
          ctx.rect(phx, phy - r * 0.28, r * 0.09, r * 0.28);
          ...
          ctx.stroke();
          ctx.beginPath();
          ctx.rect(phx + r * 0.01, phy - r * 0.22, r * 0.07, r * 0.14);
          ctx.fillStyle = "rgba(150,220,255,0.65)";
          ctx.fill();
          ctx.beginPath();
          ctx.moveTo(phx - r * 0.01, phy - r * 0.28);
          ctx.lineTo(phx + r * 0.045, phy - r * 0.34);
          ctx.lineTo(phx + r * 0.1, phy - r * 0.28);
          ctx.closePath();
        ...
        for (let mb = 0; mb < 4; mb++) {
          ...
          ctx.beginPath();
          ctx.rect(mbx, y + r * 0.1, r * 0.07, r * 0.1);
          ctx.fillStyle = ["#2244cc", "#22aacc", "#cc4422", "#22cc44"][mb];
          ctx.fill();
          ...
          ctx.fillStyle = ["#3355dd", "#33bbdd", "#dd5533", "#33dd55"][mb];
          ctx.fill();
        }
        ctx.save();
        ctx.translate(x + r * 0.36, y - r * 0.14);
        ctx.rotate(-0.8);
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.arc(0, 0, r * 0.18, Math.PI * 0.05, Math.PI * 0.95);
        ctx.closePath();
        ...
        ctx.restore();
        ctx.beginPath();
        ctx.rect(x + r * 0.38, y - r * 0.14, r * 0.04, r * 0.18);
        ctx.fillStyle = "#888890";
        ctx.fill();
        ctx.beginPath();
        ctx.rect(x + r * 0.06, y + r * 0.08, r * 0.36, r * 0.04);
        ...
        for (let ph = 0; ph < 4; ph++) {
          ...
          ctx.beginPath();
          ctx.rect(phx - r * 0.03, y - r * 0.08, r * 0.055, r * 0.16);
          ...
          ctx.stroke();
          ctx.beginPath();
          ctx.rect(phx - r * 0.025, y - r * 0.075, r * 0.045, r * 0.11);
          ctx.fillStyle = `hsl(${ph * 90},65%,45%)`;
          ctx.fill();
          for (let ic = 0; ic < 4; ic++) {
            ctx.beginPath();
            ctx.arc(
              phx - r * 0.01 + (ic % 2) * r * 0.02,
              y - r * 0.055 + Math.floor(ic / 2) * r * 0.02,
              r * 0.007,
              0,
              Math.PI * 2,
            );
          ...
          ctx.beginPath();
          ctx.arc(phx, y + r * 0.04, r * 0.007, 0, Math.PI * 2);
          ctx.fillStyle = "#555";
          ctx.fill();
        }
      }
      ...
      ctx.font = `bold ${r > 100 ? 14 : 13}px Georgia,serif`;
      ctx.textAlign = "center";
      ctx.fillText(name, x, y + r * 0.62 + 22);
      ctx.fillStyle = "rgba(255,240,180,0.7)";
      ctx.font = "11px Georgia,serif";
      ctx.fillText(`[${lbl}]`, x, y + r * 0.62 + 36);
      ctx.restore();
  ...
  const handleExplore = () => {
    ...
    setTimeout(() => setScreen("game"), 300);
  };

  if (screen === "loading") {
    return <LoadingScreen progress={progress} loadText={loadText} />;
  }
  ...
  return (
    <div className={styles.gameWrap}>
      <canvas ref={cvsRef} className={styles.canvas} />
      <div className={styles.hudWrap}>
        <div className={styles.hudTitle}>Hey, I'm Kyle Lin</div>
        <div className={styles.hudSubtitle}>
          {isTouch
            ? "Use the on-screen arrows to sail · Tap an island to visit"
      ...
      {showTutorial && (
        <div className={styles.tutorialOverlay}>
          <div className={styles.tutorialCard}>
            <div className={styles.tutorialAnchor}>⚓</div>
            <h2 className={styles.tutorialTitle}>Welcome Aboard</h2>
            <p className={styles.tutorialSubtitle}>Captain's Briefing</p>
            ...
            <div className={styles.tutorialGrid}>
              {(isTouch
                ...
                <div key={keys} className={styles.tutorialRow}>
                  <kbd className={styles.tutorialKbd}>{keys}</kbd>
                  <span className={styles.tutorialRowDesc}>{desc}</span>
                </div>
              ))}
            </div>

            <div className={styles.tutorialDivider} />
            <p className={styles.tutorialText}>
              Sail to or click on each island to explore a section of my
              portfolio.{" "}
            </p>
      ...
      {near && (
        <div
          ...
          style={{
            borderColor:
              THEMES[ISLE_DATA.find((i) => i.id === near)!.theme].accent,
          }}
        >
          {isTouch ? (
            ...
            <>
              Press <kbd className={styles.toastKbd}>E</kbd> or click to visit{" "}
              {ISLE_DATA.find((i) => i.id === near)?.name}
            </>
          )}
        </div>
      )}
      <div
        className={`${styles.dpad} ${isTouch ? styles.dpadTouch : styles.dpadDesktop}`}
      >
        {(["", "dU", "", "dL", "dD", "dR"] as const).map((id, i) => {
          ...
          return (
            <div
              ...
              className={`${styles.dpadBtn} ${isTouch ? styles.dpadBtnTouch : styles.dpadBtnDesktop}`}
            >
              {label}
            </div>
          );
