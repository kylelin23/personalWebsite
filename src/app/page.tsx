...
function shouldSkipHomePosterEntrance(){
  ...
  try{
    const flagged=window.sessionStorage.getItem('skipHomePosterEnter')==='1';
    const ref=document.referrer ? new URL(document.referrer) : null;
    const fromInternalIsland=!!ref && ref.origin===window.location.origin && ref.pathname !== '/';
...
export default function Home(){
  ...
  useEffect(()=>{
    ...
    const enterTimeout = skipEntrance ? undefined : setTimeout(() => setEntering(false), 750);
    const iv=setInterval(()=>{
      setSubtitleIndex(i=>(i+1)%SUBTITLES.length);
    },3200);
    return()=>{clearInterval(iv);if(enterTimeout)clearTimeout(enterTimeout);};
  ...
  useEffect(()=>{
    ...
    const kd=(e:KeyboardEvent)=>{
      ...
      if((e.key==='e'||e.key==='E')&&nearRef.current)visitIsland(nearRef.current.id);
    };
    ...
    function drawCat(isle:Isle,tick:number){
      ...
      if(sitting){
        ...
        ctx.beginPath();ctx.moveTo(4,-12);ctx.lineTo(3,-15);ctx.lineTo(1,-12);ctx.fillStyle='#ffaaaa';ctx.fill();
        ctx.beginPath();ctx.arc(-2,-7,1.2,0,Math.PI*2);ctx.fillStyle='#222';ctx.fill();
        ctx.beginPath();ctx.arc(2,-7,1.2,0,Math.PI*2);ctx.fillStyle='#222';ctx.fill();
        ctx.beginPath();ctx.arc(-1.5,-6.5,0.5,0,Math.PI*2);ctx.fillStyle='#fff';ctx.fill();
        ctx.beginPath();ctx.arc(2.5,-6.5,0.5,0,Math.PI*2);ctx.fillStyle='#fff';ctx.fill();
        ctx.beginPath();ctx.arc(0,-5,1.5,0,Math.PI*2);ctx.fillStyle='#ffaaaa';ctx.fill();
    ...
    function drawCompass(H:number){
      ...
      ctx.beginPath();ctx.moveTo(0,-30);ctx.lineTo(3.5,4);ctx.lineTo(-3.5,4);ctx.closePath();ctx.fillStyle='#ff4444';ctx.fill();
      ctx.beginPath();ctx.moveTo(0,30);ctx.lineTo(3.5,-4);ctx.lineTo(-3.5,-4);ctx.closePath();ctx.fillStyle='#c8a870';ctx.fill();
    ...
    function loop(){
      ...
      islesRef.current.forEach(isle=>{
        const dx=s.x-isle.x,dy=s.y-isle.y;
        const a=isle.r+18,b=isle.r*0.62+12;
